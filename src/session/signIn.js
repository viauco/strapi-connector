...
export default async function signIn(req, res) {
  try {
    ...
    const cookiesToSet = []
    cookiesToSet.push(prepareSetCookie(COOKIES.APP_CUSTOMER_TOKEN, token, { maxAge: 3600 * 24 * 30 })) // set customer token cookie for 30 days
    cookiesToSet.push(prepareSetCookie(COOKIES.APP_CUSTOMER_CART_ID, customerCartId, { maxAge: 3600 * 24 * 30 })) // set customer cart ID cookie for 30 days
